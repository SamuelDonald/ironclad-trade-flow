...
serve(async (req) => {
  ...
  if (!action || action === '') {
    ...
    // TEMPORARY: Balance updates - will be moved to dedicated balance-update function once deployed
    if ((req.method === 'PUT' && path.startsWith('/admin-operations/users/') && path.endsWith('/balances')) || 
        action === 'update-balances') {
      
      console.log('[Admin Operations] Balance update handler triggered (temporary)');
